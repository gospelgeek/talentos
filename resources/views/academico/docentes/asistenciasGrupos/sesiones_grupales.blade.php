...
<h1 id="num_sesiones" style="text-align: center;">ASISTENCIAS - {{$name->name}}<br>{{$grupo->name}} - {{$grupo->cohort->name}}<br> Total Sesiones {{count($sesiones)}} <br>{{$course->docente_name}}</h1>
                            ...
                            <td>
                                <div >                                  
                                    <div class="col-xs-12 col-sm-12">
                                        <a title="Ver Informacion" href="/lista_asistencias/{{$name->id}}/grupo/{{$grupo->id}}/session/{{$sesion->session_id}}/{{$course->docente_name}}" class="btn  btn-sm  fa fa-eye">Lista</a>    
