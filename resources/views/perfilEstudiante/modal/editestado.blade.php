      ...
      <div class="modal-body">
        <div class="container-fluid">
          <div class="row col-xs-8 col-sm-8" style="float: left;">
            ...
            <div class="col-xs-12 col-sm-6 form-group {{$errors->has('body') ? 'has-errors' : ''}}">
              {!!Form::select('CMotivo', $motivos, $verDatosPerfil->withdrawals ? $verDatosPerfil->withdrawals->id_reasons : null,['id'=>'CMotivo','class'=>'form-control','required','placeholder'=>'* Motivos'])!!}  
            </div>       
            <div class="col-xs-12 col-sm-6 form-group {{$errors->has('body') ? 'has-errors' : ''}}">    
              {!!Form::text('url',$verDatosPerfil->withdrawals ? $verDatosPerfil->withdrawals->url : null,['id'=>'CUrl','class'=>'form-control','placeholder'=>'*URL'])!!}
              <a href="{{$verDatosPerfil->withdrawals ? $verDatosPerfil->withdrawals->url : null}}" target="blank" class="fa fa-external-link" id="CBoton">Enlace Documento</a>
            </div>
          </div>
          <div class="row">
